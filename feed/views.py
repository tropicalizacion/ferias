...
def feed(request):
    ...
    alerts = Alert.objects.filter(start_date__lte=today, end_date__gte=today)

    feed_list = sorted(
        chain(events, news, alerts), key=attrgetter("created_at"), reverse=True
    )
    ...
    context = {"is_marketplace_admin": is_marketplace_admin, "feed_list": feed_list}
